...
if(!empty($_GET['did'])){
     $did=$_GET['did']; 
      $id=$_GET['id']; 
    $d = "DELETE FROM `Case_Studies_Challenge` WHERE id='$did'";
    $d_r = mysqli_query($conn,$d);
    ?>
    <script>window.location.href="challenge.php?id=<?php echo $id; ?>";</script>
    <?php
}


if(!empty($_GET['li_did'])){
     $did=$_GET['li_did']; 
      $id=$_GET['id']; 
    $d = "DELETE FROM `Case_Studies_Challenge_li` WHERE id='$did'";
    $d_r = mysqli_query($conn,$d);
    ?>
    <script>window.location.href="challenge.php?id=<?php echo $id; ?>";</script>
    <?php
...
{   
    $sid = $_GET['sid'];
    $status = $_GET['status'];
    $query = "UPDATE `Case_Studies_Challenge` SET `status`='$status' WHERE id=$sid";
...
if(!empty($_POST['submit'])){
    ...
    $context_number = $_POST['challengeNo'];
    $colour = $_POST['clr'];
    $content = addslashes($_POST['case_content']);
   
    
    $query = "INSERT INTO `Case_Studies_Challenge`(`Case_Studies_id`, `number`, `color`, `content`, `status`) VALUES ('$id','$context_number','$colour','$content','Active')";
...
<?php
if(!empty($_POST['list_submit'])){
    
    $Case_Studies_id = $_POST['Case_Studies_id'];
    $Challenge_id = $_POST['Challenge_id'];
    $content = addslashes($_POST['content']);
   
    
    $query = "INSERT INTO `Case_Studies_Challenge_li`(`Case_Studies_id`, `Challenge_id`, `content`, `status`) VALUES ('$Case_Studies_id','$Challenge_id','$content','Active')";
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-12 row">
                            <div class="col-lg-6">
                                <label for="address" class="form-label ">Challenge Number<strong style="color:red;">*</strong></label>
                                <input type="text" name="challengeNo" class="form-control mb-3" placeholder="Enter Service Heading" >
                            </div>
                            ...
                            <div class="col-12">
                                <div>
                                    ...
                                    <div id="example_wrapper" class="dataTables_wrapper no-footer">
                                        ...
                                        <?php
                                            $caseq= "SELECT * FROM `Case_Studies_Challenge` where Case_Studies_id='$id'";
                                          ...
                                          ?>                                                                                                
                                            <tr class="odd">
                                                ...
                                                <td><?php echo $case_r['created_date']; ?></td>
                                                 ...
                                                 <td><button type="button" class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#de<?php echo $case_r['id'];?>" title="View Details">Add List</button></button>
                                                    <div id="de<?php echo $case_r['id'];?>" class="modal fade flip" tabindex="-1" aria-labelledby="flipModalLabel" aria-hidden="true" style="display: none;">
                                                <div class="modal-dialog modal-dialog-centered">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h6 class="modal-title" id="flipModalLabel"><?php echo $case_r['content']; ?></h6>
                                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                        </div>
                                                        <div class="modal-body">
                                                            <div class="card" style=''>
                                                                <table class="table dataTable no-footer" id="examp le" style='border:1px solid black'>
                                                                    <thead style="background-color: black;color:white">
                                                                        <tr>
                                                                            <th>Content</th>
                                                                            <th>Delete</th>
                                        ...
                                        <?php
                                        $Ch_id=$case_r['id'];
                                              $q_chli = "SELECT * FROM Case_Studies_Challenge_li WHERE Case_Studies_id='$id' AND Challenge_id='$Ch_id'";
                                                $q_chli_run = mysqli_query($conn,$q_chli);
                                        ?>
                                                                    <tbody>
                                                                        <?php foreach($q_chli_run as $q_chli_r){ ?>
                                                                        <tr>
                                                                            <td><?php echo $q_chli_r['content']; ?></td>
                                                                            <td><a href="?li_did=<?php echo $q_chli_r['id']; ?>&id=<?php echo $id; ?>" onclick='return checkdelete(1);' class="btn btn-sm btn-danger edit-item-btn ms-2">Delete</a></td>
                                                                        </tr>
                                                                        <?php } ?>
                                                                    </tbody>
                                                                </table>
                                                            ...
                                                            <div class="col-lg-12 row">
                                                                <div class="col-lg-12">
                                                                    <input type='text' name='Case_Studies_id' value='<?php echo $id; ?>' style='display:none'>
                                                                    <input type='text' name='Challenge_id' value='<?php echo $case_r['id']; ?>' style='display:none'>
                                                                    <label for="address" class="form-label ">Add List<strong style="color:red;">*</strong></label>
                                                                    <input type="text" name="content" class="form-control mb-3" placeholder="Enter content" >
                                                                </div>
                                                                 <div class="col-lg-12">
                                                                     <center>
                                                                    <input type="submit" class="btn btn-success w-sm btn-sm"  name="list_submit" value="Submit">
